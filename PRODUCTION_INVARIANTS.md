...
**Status.** `implemented` — The request timeout parser caps the total budget, urllib3 receives a combined total timeout, and X create requests do not follow redirects. X API and upload configuration rejects paths, queries, fragments and user information; create transport also fails closed if literal and prepared route classifications disagree. X post and v2 media-upload paths publish and revalidate exact durable transport barriers before transmission and preserve those barriers for every post-transmission outcome not proved successful or definitely unsent. The legacy v1.1 media helper refuses before transport, so an ambiguous v2 outcome cannot become a second upload through a different endpoint. The separate xAI provider base deliberately retains its versioned /v1 path.

**Verification.** `verified` — Tests inspect the combined timeout object, reject non-finite or oversized values, inject network failures, generic non-2xx responses and redirects, and prove ambiguous writes and made-with-AI rejection text are not retried. Literal-process media tests cover every v2 upload hard-exit boundary and prove a fresh process remains globally blocked, while a focused regression proves the legacy v1.1 helper refuses before any transport call. Configuration and unit regressions reject pathful or otherwise non-origin X bases, preserve the versioned xAI provider base and require prepared/literal tweet and media route classifications to agree before transport.
