"""Validate conversational reply receipts and project their in-memory values.

Root adapters supply current validators, canonical JSON/date/clock helpers,
legacy strategy versions, logger and exception authority on every call. Current
and frozen recovery validation dispatch through those root callbacks. Original
shallow copies, source hashes, timing rules and error boundaries are preserved.

Durable receipt loading, writing, promotion, removal and retirement, state
application/reconciliation, counters, mention watermarks and outbound authority
remain in their existing locations. Import uses only the standard library and
...
def _conversational_reply_receipt_is_semantically_valid(
    ...
    *,
    ...
    _legacy_ai_reply_receipt_draft_is_valid: Callable,
    ai_reply_receipt_draft_is_valid: Callable,
    _LEGACY_TESTED_REPLY_STRATEGY_VERSION: str,
    _LEGACY_AI_FIRST_REPLY_STRATEGY_VERSION: str,
    conversational_sending_receipt_from_confirmed: Callable,
...
def confirmed_reply_receipt_is_semantically_valid(
    data: dict,
    *,
    _conversational_reply_receipt_is_semantically_valid: Callable,
) -> bool:
...
def sending_reply_receipt_is_semantically_valid(
    data: dict,
    *,
    _conversational_reply_receipt_is_semantically_valid: Callable,
) -> bool:
...
def _legacy_confirmed_reply_receipt_is_semantically_valid(
    data: dict,
    *,
    _conversational_reply_receipt_is_semantically_valid: Callable,
) -> bool:
...
def _legacy_sending_reply_receipt_is_semantically_valid(
    data: dict,
    *,
    _conversational_reply_receipt_is_semantically_valid: Callable,
) -> bool:
...
def bind_conversational_reply_attempt_time(
...
def conversational_reply_confirmation_epoch(
    receipt: dict,
    *,
    receipt_int: Callable,
