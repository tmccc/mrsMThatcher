...
Importing this module does no runtime work and retains no callbacks or cache.
"""
...
def original_editorial_concepts(
    value: object,
    *,
    normalise_tag: Callable[[object], str],
    original_editorial_concepts: Callable[[object], set[str]],
    ...
    if "_and_" in tag:
        ...
        return concepts
    concept = synonym_to_concept.get(tag)
    if concept in affinity_concepts:
        return {concept}
    return set()


def original_editorial_quote_concepts(
...
def original_editorial_image_concepts(
    editorial: dict | None,
    *,
    as_string_list: Callable[[object], list[str]],
    original_editorial_concepts: Callable[[object], set[str]],
...
def original_editorial_avoid_concepts(
    editorial: dict | None,
    *,
    as_string_list: Callable[[object], list[str]],
    original_editorial_concepts: Callable[[object], set[str]],
...
def original_editorial_quote_dimension_profile(
    quote_analysis: dict | None,
    *,
    dimensions: list[str],
    normalise_tag: Callable[[object], str],
    as_string_list: Callable[[object], list[str]],
    original_editorial_quote_concepts: Callable[[dict | None], set[str]],
) -> dict[str, float]:
...
def validate_original_editorial_item(
    basename: str,
    entry: dict,
    ...
    *,
    ...
    current_image_sha256: Callable[[str], str],
    original_editorial_numeric: Callable[..., float],
) -> dict:
...
def load_original_editorial_analysis(
    *,
    analysis_file: str | Path,
    analysis_cache: dict[str, dict],
    analysis_kind: str,
    schema_version: int,
    current_image_paths: Callable[[], list[str]],
    validate_original_editorial_item: Callable[[str, dict, dict[str, str]], dict],
    generated_image_origin_quote_hash: Callable[[str], str | None],
) -> dict[str, dict]:
    ...
    if data.get("analysis_kind") != analysis_kind:
...
def validate_original_editorial_shadow_startup(
    *,
    enabled: bool,
    load_original_editorial_analysis: Callable[[], dict[str, dict]],
    analysis_file: str | Path,
...
def original_editorial_shadow_score(
    ...
    *,
    ...
    default_weight: float,
    default_max_abs_adjustment: float,
    original_editorial_quote_dimension_profile: Callable[[dict | None], dict[str, float]],
    original_editorial_numeric: Callable[..., float],
    original_editorial_quote_concepts: Callable[[dict | None], set[str]],
    original_editorial_image_concepts: Callable[[dict | None], set[str]],
    original_editorial_avoid_concepts: Callable[[dict | None], set[str]],
) -> tuple[float, dict]:
    ...
    max_abs_adjustment = default_max_abs_adjustment if max_abs_adjustment is None else float(max_abs_adjustment)
...
def original_editorial_shadow_result(
    ...
    *,
    selection_phase: str,
    load_original_editorial_analysis: Callable[[], dict[str, dict]],
    original_editorial_shadow_score: Callable[..., tuple[float, dict]],
    default_weight: float,
    default_max_abs_adjustment: float,
