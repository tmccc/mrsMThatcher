...
- Candidate identity rule: The exact release base for this ledger is 4e548b0a5723a1f0c75e9646953b3f92c7db89ad. Any descendant candidate identity is supplied only by the frozen-candidate release attestation and is deliberately not embedded here. Repairs committed by that base are recorded with fix-bound evidence; worktree proposals after that base, including proposals for DEF-0052 through DEF-0065, remain external and unfixed in ledger truth until merge and post-merge regeneration.
...
- Regeneration rule: After a merge or deployment changes any recorded defect, invariant or deployment status, regenerate and revalidate this ledger from the new exact release base before using it for another release attestation. No uncommitted or post-4e548b0a candidate implementation, test or proposed repair may change DEF-0044, DEF-0045 or DEF-0052 through DEF-0065 from their recorded status; closure requires an ancestor of the regenerated cut-off with fix-bound tests and chronology.
...
| `DEF-0017` | runtime-defect | `mrsMThatcher2.py`; `mrsMThatcher.local.json`; `mrsMThatcher.control.json`; `tests/test_fail_safe_bootstrap_and_control.py` | `configuration-and-pause-controls` | unknown — No complete archive of production configuration or control documents establishes whether a duplicate name, non-finite number or exactly fractional timestamp altered live behavior. Evidence: Default json.load behavior at the exact 4e548b0a cutoff reproduces last-wins parsing for duplicate local-configuration and runtime-control names and accepts NaN, Infinity and -Infinity constants; ordinary float decoding can also turn finite-looking exponent tokens into infinities or round mathematically fractional runtime-control timestamps to integral binary floats. |
...
| `DEF-0058` | runtime-defect | `mrsMThatcher2.py`; `historical_context_outbox.py`; `tests/test_x_write_outcome_conservatism.py`; `tests/test_production_consistency_incident.py` | `historical-context-reply`; `regular-quote-image-post`; `daily-meme-post`; `conversational-reply`; `media-upload`; `cross-cutting-remote-write-barrier` | false — This establishes both that an outbox-only possibly transmitted attempt could fail to block another lane at the cutoff and that the first descendant global-barrier repair could strand a safely locally reconcilable terminal outcome. It does not establish that a deployed historical-context reply or unrelated remote write was repeated. Evidence: The cutoff barrier gap was reproduced with isolated outbox state after removing independent source-receipt and transport-journal authority and making the historical-context runtime unavailable. Exact review of rejected descendant candidate b5e62c0 then retained a sole source-bound remote-started row plus exact completed history after receipt retirement and found that the new global blocker made its existing local-only reconciliation path unreachable. No remote transport, network, production state, X or provider was accessed. |
...
| `DEF-0063` | runtime-defect | `mrsMThatcher2.py`; `mrsMThatcher.control.json`; `tests/test_fail_safe_bootstrap_and_control.py` | `cross-cutting-runtime-control`; `all-remote-write-lanes` | false — This establishes that a locally published pause could be hidden by cache reuse; it does not establish that a deployed operator pause was missed or that a live remote action occurred. Evidence: The stale-cache path was reproduced with an isolated control file by loading an unpaused document, rewriting the same inode with equal-length paused content, synchronising it and restoring the original mtime. Device, inode, size and mtime remained equal while the cutoff loader returned the cached unpaused value. No production control, service, network, X or provider was accessed. |
| `DEF-0064` | runtime-defect | `mrsMThatcher2.py`; `tests/test_fail_safe_bootstrap_and_control.py` | `command-line-dispatch`; `test-cycle`; `test-main-tick`; `test-quote-post`; `test-meme-post`; `production-bootstrap` | false — This establishes that late environment mutation could authorise or revoke a test command inconsistently with the globals constructed during import; it does not establish that a deployed process used that path or performed a remote action. Evidence: The authority drift was reproduced in literal isolated child processes for all four test modes and their direct entry points by importing with MRS_TEST_MODE absent or present, changing the environment after import, and instrumenting bootstrap, lock and operational boundaries. No production path, service, network, X or provider was accessed. |
| `DEF-0065` | runtime-defect | `tools/activate_remote_write_safety_protocol.py`; `tests/test_remote_write_safety_second_restart.py` | `offline-protocol-activation`; `cross-cutting-remote-write-barrier` | false — This establishes an ambiguous stopped-tool instruction boundary; it does not establish that a production activation targeted the wrong namespace or that runtime fail-closed checks were bypassed. Evidence: The parser ambiguity was reproduced without activation by passing unique prefixes for documented long options and by repeating project identity options with different values. The cutoff parser accepted abbreviations and retained the later repeated value. No project namespace, production state, service, network, X or provider was accessed. |
...
| `DEF-0017` | 2026-08-02 | unknown | Review of rejected descendant candidate b5e62c0 reconfirmed that local configuration and runtime control still used permissive JSON decoding and expanded the bounded defect to nested duplicate names, explicit non-finite constants and exponent overflow to non-finite floats. | Exact 4e548b0a source reproduction and rejected-b5 strict runtime-document review |
| `DEF-0017` | 2026-08-02 | unknown | Review of rejected descendant candidate d2ee28e showed that runtime-control timestamps still lost exact decimal semantics before integrality validation. | Exact 4e548b0a source reproduction and rejected-d2ee runtime-control review |
...
| `DEF-0058` | 2026-08-02 | unknown | Exact review of rejected descendant candidate b5e62c0 showed that its remote-started outbox blocker ran before the only local reconciler which could consume a sole exact source-bound completed-history outcome after the receipt and journal had been retired. | Receipt-absent pre-barrier reachability review with exact completed history and multiple-risky-row refusal |
...
| `DEF-0063` | 2026-07-11 | `4a4afb4f` | The runtime-control cache was introduced with metadata, rather than current document bytes, as cache authority. | Git introduction of _CONTROL_CACHE and the early return in load_control |
| `DEF-0063` | 2026-08-02 | `4e548b0a` | Exact-cutoff review rewrote an equal-length control document in place, restored its mtime and observed the cached unpaused value survive despite different current bytes. | Isolated same-inode, equal-size, restored-mtime control-cache reproducer |
| `DEF-0064` | 2026-07-01 | `36edf036` | Initial test-cycle and test-post entry points used live MRS_TEST_MODE environment reads as command authority. | Initial bot source and integration harness |
| `DEF-0064` | 2026-07-02 | `4eb54652` | The test-main-tick entry point extended the same mutable authority pattern to the fourth test CLI mode. | Introduction of run_test_main_tick and shared require_test_mode use |
| `DEF-0064` | 2026-08-02 | `4e548b0a` | Exact-cutoff review mutated MRS_TEST_MODE after import in both directions for every test flag and direct entry point and observed authority disagree with import-time configuration and bootstrap ordering. | Literal-process four-mode late-enable and late-disable matrix |
| `DEF-0065` | 2026-07-31 | `5b0b6108` | The stopped activation CLI was introduced with default argparse long-option abbreviation and repeated-option behavior. | Initial tools/activate_remote_write_safety_protocol.py parser |
| `DEF-0065` | 2026-08-02 | `4e548b0a` | Exact-cutoff parser review supplied abbreviated long options and conflicting repeated project identity options and observed acceptance rather than a usage refusal. | Isolated activator exact-option and duplicate-destination matrix |
...
| `DEF-0017` | active | high | Configuration and pause controls accepted ambiguous names and unsafe numeric semantics | `INV-CONFIG-001`, `INV-PAUSE-001` | `36edf036`, bounded | unfixed | active at the ledger evidence cut-off; production presence not established |
...
| `DEF-0063` | active | high | Runtime-control cache trusted metadata instead of stable document content | `INV-PAUSE-001` | `4a4afb4f` | unfixed | observed in production `be882e81` |
| `DEF-0064` | active | critical | Test CLI modes obtained authority from mutable post-import environment state | `INV-PROC-001` | `36edf036`, bounded | unfixed | observed in production `be882e81` |
| `DEF-0065` | active | high | Stopped activation CLI accepted abbreviated and repeated safety-critical options | `INV-TXN-RECEIPT-001`, `INV-PROC-002` | `5b0b6108` | unfixed | not deployed; absent from observed production `be882e81` |
...
At `4e548b0a`, command modes were independent membership tests over `sys.argv`.
Unknown and positional arguments could fall through to the production daemon,
duplicates were indistinguishable from one flag and simultaneous documented
...
At `4e548b0a`, the stopped remote-write protocol activator's exact refused-state
inventory omitted `.mrsMThatcher.initialising.json`. An interrupted first
installation with the activator's narrower required files could therefore
receive a schema-3 activation pair and misleading clean-state attestation.
Runtime installation validation still rejected the sentinel, so this produced
false assurance and fail-closed availability obstruction rather than opening a
remote-write lane. The current uncommitted proposal refuses the sentinel before
any activation mutation for absent, pre-ledger, current and audit-only states.
No fixing commit is inside the ledger boundary; the defect remains active and
undeployed.

### DEF-0063 — Runtime-control cache trusted metadata instead of stable document content

At `4e548b0a`, `load_control()` could reuse its last valid document solely from
pathname, device, inode, size and modification time. An equal-length in-place
rewrite with its modification time restored could therefore leave an earlier
unpaused value authoritative even though the current bytes requested a pause.
The current uncommitted proposal rereads a bounded no-follow regular-file
snapshot on every poll, binds descriptor and pathname identity, requires two
identical exact-length reads, includes the content SHA-256 in the cache
signature, and preserves an isolated last-valid copy under a fail-closed global
pause on any non-initial-absence failure. No fixing commit is inside the ledger
boundary, so the defect remains active and undeployed.

### DEF-0064 — Test CLI modes obtained authority from mutable post-import environment state

At `4e548b0a`, all four operational test-only commands reread
`MRS_TEST_MODE` after import and only after bootstrap or other establishment
work. Late environment mutation could consequently make command authority
disagree with the paths, endpoints and safety globals already fixed during
module import. The current uncommitted proposal captures test authority in the
standard-library-only import gate and uses that immutable value in early script
validation, callable dispatch and every direct test entry point before
bootstrap. Literal-process regressions cover both mutation directions for all
four modes. No fixing commit is inside the ledger boundary, so the critical
defect remains active and undeployed.

### DEF-0065 — Stopped activation CLI accepted abbreviated and repeated safety-critical options

At `4e548b0a`, the stopped protocol activator retained argparse's default
long-option abbreviation and repeated-option last-value behaviour. A unique
prefix or conflicting repeated project/attestation option could therefore be
accepted instead of representing one exact reviewed instruction vector. The
current uncommitted proposal disables abbreviation and rejects a second
occurrence of any option destination, including aliases and `--option=value`,
before parsing or activation work while preserving the complete documented
invocation. No fixing commit is inside the ledger boundary, so the defect
remains active and undeployed.
...
- `DEF-0044`, `DEF-0045`, and `DEF-0052` through `DEF-0065`: introduction and
