...
| `mrs_bot_reply_receipt_values.py` | Conversational receipt validation, send-template preparation, source reconstruction, pagination provenance and attempt/confirmation times | `ReplyReceiptValues` owns current/legacy lifecycle dispatch and calls its validation/projection methods directly. Preserves shallow source references and hashes; conservative confirmation uses the supplied clock/logger. Construction is inert; durable I/O and retirement authority remain external. Fixed scalar validation and frozen legacy strategy identifiers come directly from their inert owners; clocks, date/epoch policy, draft validation and errors remain current inputs. |
...
| `mrs_bot_x_request.py` | Authenticated X requests and response handling | Enforces route/body/pause/authority checks before transport, handles diagnostics and provider health, and closes proof scopes. Fixed JSON/path/exception primitives are owned locally; bearer transport is read-only |
