...
| `mrs_bot_local_config.py` | Strict runtime JSON and local-configuration loading/coercion | `LocalConfiguration` owns bounded stable reads, strict decoding, migration/coercion and complete override validation. Runtime validators remain supplied per call; source defaults, applying overrides and bootstrap remain in the root. Fixed parsing uses local standard-library helpers. Fixed snapshot copying is local; current defaults, validation, filesystem authority and errors remain injected |
| `mrs_bot_runtime_configuration.py` | Runtime configuration paths, validation, application and credential checks | Owns path containment, historical-context schema checks, numeric bounds and cross-field validation. Applies validated values through root adapters and checks credentials. Root path-guard aliases preserve startup order; configuration storage and source-default snapshots remain in the root. Import is inert. |
| `mrs_bot_self_test.py` | Local self-test orchestration and result logging | Owns local JSON decoding; current paths, configuration, loaders and logging remain supplied. Reports local validation failures without entering the production loop or calling providers. |
