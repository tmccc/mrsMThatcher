...
| `mrs_bot_reply_state.py` | Pending-draft validation, typed recovery and confirmed conversational history | Recovery returns a complete decision or an explicit discarded-draft result, with absence separate from validation failure. Preserves current validation, deep copies, history ordering and zero-provider-call recovery; durable storage and transport remain separate |
