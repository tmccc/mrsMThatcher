...
| `mrs_bot_state_persistence.py` | State documents, `StateBackups`, and canonical generation publication | Backup rotation owns stable copying and temporary path construction; canonical publication retains reader checks, lock authority, commit proofs and the save-before-latest-backup boundary; load/save use the fixed value-free state summary directly and generation serialization remains in its owner |
| `mrs_bot_state_candidate_validation.py` | State candidate normalization, reader compatibility and meme schedule validation | Scalar and mention normalization resolve their existing StateValues and MentionAuthority owners per operation. Legacy/current history collections share ordered validation and shallow retention; recovery callbacks, overflow resets, legacy evaluation retirement and quarantine pruning retain their original order. Fixed receipt-commit grammar comes from the state-generation owner. |
| `mrs_bot_state_loading.py` | Durable state loading and ordered recovery selection | Separates replica selection, rollback-fence validation and selected-candidate recovery reporting. Primary ambiguity stops loading; unusable unneeded backups retain their warning path. Sealed-generation collision checks, newest-replica choice and reporting-before-durable-repair order are preserved. |
