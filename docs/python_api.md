...
| `mrs_bot_reply_generation.py` | Typed reply evaluation and outcome accounting | Fixed health/local classification is used directly by generation and reply cycles; `ReplyGeneration` owns evaluation and decision telemetry. Coordinates media, history, evidence/pipeline and model transport through current callbacks. Returns the full PipelineResult and preserves cooldown ordering, prior-429 accounting, telemetry and distinct provider-health/candidate dispositions. No import-time runtime work |
