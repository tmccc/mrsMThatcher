...
| `mrs_bot_state_candidate_validation.py` | State candidate normalization, reader compatibility and meme schedule validation | Scalar and mention normalization resolve their existing StateValues and MentionAuthority owners per operation; recovery callbacks, overflow resets, legacy evaluation retirement and quarantine pruning retain their original order; fixed receipt-commit record grammar comes from the state-generation owner beside its writer and commit proofs |
...
| `mrs_bot_post_creation.py` | Receipt-bound media upload, public-post creation and confirmed-media handoff | Owns fixed media metadata construction, path values and post-ID validation used directly by upload, request and handoff checks; separates prepared-receipt validation and payload projection before source-bound journal orchestration and preserves pause, rejection, ambiguity and SIGINT boundaries |
...
| `mrs_bot_runtime_service_initialisation.py` | Runtime health, lazy reply evidence and historical semantic-gate initialization | Initializes services on explicit calls using coordinator-owned caches and logger; semantic review and live context formatting share the same ordered option projection |
