...
| `mrs_bot_mention_discovery.py` | Mention discovery, durable queue access and watermark retirement | `MentionQueue` owns recovery, ordered access and monotonic retirement through current authority, sorting and save boundaries. Fetched-page traversal uses fixed copying, cursor hashing and the shared backlog guard directly; transient lookup failures retain the queue |
