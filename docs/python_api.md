...
| `mrs_bot_daily_meme.py` | Daily meme assets, calendar scheduling and transactional posting | `MemeCatalog` owns discovery, summaries and history-reset selection; `MemeSchedule` owns calendar, fallback and quote-anchor operations. `MainPostPublication` shares durable publication; the lane retains named diagnostics, preparation exception boundaries, schedule projections and emergency state recovery |
...
| `mrs_bot_reply_receipt_values.py` | Conversational receipt validation, send-template preparation, source reconstruction and attempt/confirmation times | `ReplyReceiptValues` owns current/legacy lifecycle dispatch and calls its validation/projection methods directly. Preserves shallow source references and hashes; conservative confirmation uses the supplied clock/logger. Construction is inert; durable I/O and retirement authority remain external. Fixed scalar validation, mention continuation grammar and frozen legacy strategy identifiers come directly from their inert owners; clocks, date/epoch policy, draft validation and errors remain current inputs |
...
| `mrs_bot_receipt_primitives.py` | Receipt scalar values, calendars and confirmation times | Fixed identity/basename/scalar checks use owned operations directly; epoch bounds remain current inputs. `ReceiptDates` owns ambient and daily-cap dates with safe conversions. Receipt-bound timezone validation/conversion remain separate late-bound operations; conservative confirmation fallback remains explicit |
| `mrs_bot_runtime_state_helpers.py` | Runtime state defaults, load maintenance and scheduling values | Creates fresh defaults and mutates supplied schedule fields; load maintenance calls `ApiCooldowns.clear_expired` directly before priority sanitisation. Epoch validation and lazy scheduling use current callbacks; `QuoteSchedule` owns current delay selection and optional-save scheduling, with fixed field application local |
