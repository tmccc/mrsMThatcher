...
| `mrs_bot_reply_generation.py` | Reply pipeline evaluation, failure dispositions and decision telemetry | `ReplyGeneration` separates material-image failure disposition from provider evaluation and health accounting. Failed media uses the original target/visible-turn snapshot and records the zero-call result before warning, without history/provider access. Current pipeline, evidence, media, error vocabulary and telemetry boundaries remain supplied; result references and cooldown separation are preserved |
