...
| `mrs_bot_quote_candidates.py` | Ordinary quotation eligibility, candidate preparation and weighted selection | `QuoteCandidates` owns source preparation, date/quality weights and quote-cycle selection with current metadata, source and candidate-hash capabilities, including per-bot simulation caches. The shared research loader uses fixed canonical quotation identity directly. Candidate references, history resets, attribution checks and shared RNG order are preserved; import and construction are inert |
| `mrs_bot_image_selection.py` | Original-image eligibility, selection and quotation-image pair retries | `ImageSelection` separates eligible-cycle preparation from scored-pool choice, editorial application and diagnostics. Fixed topic weights, calendar eligibility and formatting are direct. Pair retries bind current metadata, scoring and editorial policy after each quotation choice; row references, comparison reuse, migration saves, forced reset, exception scope and shared RNG order are preserved |
| `mrs_bot_quote_posting.py` | Ordinary quotation posting and local recovery | Retains selection, history rollback, context obligations and recovery projections. A cycle-bound `MainPostPublication` owns shared publication steps; confirmed persistence and receipt retirement use regular-post completion |
...
| `mrs_bot_used_history.py` | `UsedHistory`: durable quote and image history | Owns stable loading, source-verified migration and conditional rewrites; the image normalization algorithm also supports the historical simulator's separate corpus-proof policy. Fixed JSON, regex, hash and basename operations are owned here; source/corpus proofs, paths, durable writes and current metadata callbacks retain their existing boundaries. |
