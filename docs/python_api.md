...
| `mrs_bot_main_post_receipts.py` | Main-post receipt validation and bound regular/meme schedule materialization | Validates lane/schema/source lineage and canonical hashes with fixed scalar, identity, basename and quote-hash helpers from their owners; copying, hashing, regex and calendar arithmetic are local; replay uses bound calendar/timezone data, including DST transitions |
