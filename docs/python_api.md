...
| `mrs_bot_image_scoring.py` | Quotation/image lexical matching, topic IDF, visual energy, seasonal exclusion and component scoring | Fixed regex, tag, list, corpus, visual-energy and calendar-window helpers are used directly; current phrase callbacks, token policy and penalties retain their existing runtime boundaries |
| `mrs_bot_original_editorial.py` | Original-image editorial metadata, concepts, scoring and selection comparisons | `OriginalEditorial` owns recursive concepts, validation/load/cache, profiles, bounded scores and comparisons, using fixed tag/list normalization and generated-image identity directly. It calls the invocation's `AssetMetadata` for image discovery and hashing. Current vocabulary, dimensions, weighting and cache references bind per root call; comparison reuse and row/diagnostic copies preserve existing selection and logging behavior |
| `mrs_bot_asset_metadata.py` | Asset loading, quote overrides, identity checks and current catalog discovery | `AssetMetadata` owns JSON, quote/image/meme loaders, override application and metadata lookups; pure normalization/hash/merge helpers call locally. One instance is shared across each selection graph. Current paths, glob, logger, image checksum and exception authority bind without I/O. Preserves JSON-copy semantics, validation order, nested references and native failure boundaries |
| `mrs_bot_quote_candidates.py` | Ordinary quotation eligibility, candidate preparation and weighted selection | `QuoteCandidates` owns source preparation, date/quality weights and quote-cycle selection and calls `AssetMetadata` directly for quote loading, validation, lookups and eligibility-manifest JSON. Candidate hashing remains explicit for per-bot simulation caches. The shared research loader uses fixed canonical quotation identity directly. Candidate references, history resets, attribution checks and shared RNG order are preserved; import and construction are inert |
| `mrs_bot_image_selection.py` | Original-image eligibility, selection and quotation-image pair retries | `ImageSelection` calls `AssetMetadata`, `UsedHistory`, `OriginalEditorial` and `QuoteCandidates` directly. It separates eligible-cycle preparation from scored-pool choice, editorial application and diagnostics, and owns bounded pair retries. Fixed topic weights, calendar eligibility and formatting are direct. Row references, comparison reuse, migration saves, forced reset, exclusions, exception scope and shared RNG order are preserved |
| `mrs_bot_quote_posting.py` | Ordinary quotation posting and local recovery | Receives invocation-scoped `ImageSelection`, `MainPostReceipts`, `MainPostReceiptValues` and `TweetLookupCache` owners and calls them directly for selection, materialization, storage and cache updates while retaining history rollback, context obligations and recovery projections. A cycle-bound `MainPostPublication` owns shared publication steps; confirmed persistence and proof-gated receipt retirement use regular-post completion |
...
| `mrs_bot_daily_meme.py` | Daily meme assets, calendar scheduling and transactional posting | `MemeCatalog` owns discovery, summaries and history-reset selection; `MemeSchedule` owns calendar, fallback and quote-anchor operations. Transactional posting also receives `MainPostReceipts`, `MainPostReceiptValues` and `TweetLookupCache` directly. `MainPostPublication` shares durable publication; named diagnostics, random-draw order, preparation exception boundaries, schedule projections, proof-gated retirement and emergency state recovery remain unchanged |
| `mrs_bot_legacy_reply_validation.py` | Frozen draft validation for retired tested-pipeline, AI-first and single-Sol schemas | Owns fixed schemas, strategy definitions and pure hash/claim validators directly; current single-sol context checks and size limits remain external. A frozen sending draft cannot authorize a new send; fixed post-ID and frozen context validators are local |
| `mrs_bot_reply_drafts.py` | Pending reply draft validation, storage, recovery and retirement | `ReplyDrafts` owns the full draft lifecycle and calls its `ReplyHistory` and `ReplyGeneration` owners directly. Storage checks current confirmed replies before retaining a fresh draft. Recovery's local-validation failure retires the draft before generation records the dedicated zero-call diagnostic result. Preserves draft copies, retained references, recovery/error distinctions and target-wide clearing; durable saves and receipt authority remain external |
| `mrs_bot_reply_history.py` | Confirmed reply recording, retention and history selection for evaluation and recovery | `ReplyHistory` owns filtering, deduplication, ordering, context exclusions and history inputs. Evaluation uses the target timestamp; fresh draft storage and recovery use current time. Mutates explicit caller state without durable saves; construction is inert and captures current limits separately from the fixed recent-reply default. Fixed string post-ID validation uses receipt primitives directly |
...
| `mrs_bot_reply_generation.py` | Reply pipeline evaluation, failure dispositions and decision telemetry | `ReplyGeneration` receives `ReplyMedia`, `ReplyHistory`, `ReplyModelTransport` and `ApiCooldowns` directly and calls their collection, selection, transport, active-cooldown and provider-error operations without root relays. Failed media uses the original target/visible-turn snapshot and records the zero-call result before warning, without history/provider access. Current pipeline, evidence, remote-operation pause and telemetry boundaries remain supplied; result references, actual-call accounting and cooldown scope/save ordering are preserved |
| `mrs_bot_reply_model_transport.py` | Bounded Responses requests, retries and provider failure metadata | `ReplyModelTransport` separates transport-error classification from retry control, preserving first-429 cooldown evidence, exception causes and health-progress completion. Current transport, credentials, clock, pause and error authority remain supplied. Retries still require proved non-execution; response ownership, decoding and envelope checks preserve closure and error ordering |
| `mrs_provider_request_records.py` | Exact private provider-request capture and verification | Serialises the final application request once, publishes an owner-private durable gzip JSON record before transmission, and provides bounded integrity-checked readers for offline exporters. It stores no credentials or HTTP headers and performs no import-time I/O |
| `mrs_bot_reply_cycle_interfaces.py` | Typed reply-cycle settings, prepared context/media results and evaluation, persistence, discovery and backlog callbacks | Narrow protocols describe normal discovery, explicit-budget backlog re-entry and grouped quote discovery. Per-invocation values are supplied by the coordinator; re-exports the inert `ReplyCycleDelivery` behavior owner; no I/O |
...
| `mrs_bot_reply_receipt_values.py` | Conversational receipt validation, send-template preparation, source reconstruction and attempt/confirmation times | `ReplyReceiptValues` owns current/legacy lifecycle dispatch and calls invocation-scoped `ReplyDrafts` and `ReceiptDates` owners directly for draft authority, safe attempt time and London confirmation dates. Preserves shallow source references and hashes; conservative confirmation uses the supplied clock/logger. Construction is inert; durable I/O and retirement authority remain external. Fixed scalar validation, mention continuation grammar and frozen legacy strategy identifiers come directly from their inert owners |
| `mrs_bot_reply_reconciliation.py` | Confirmed reply state application, shared completion, restart reconciliation and emergency completeness | Confirmed application calls `ReplyReceiptValues`, `MentionAuthority`, `MentionQueue`, `ReplyDrafts`, `TweetLookupCache`, `ReplyHistory`, accounting and clarification owners directly, sharing one invocation-scoped `ReceiptDates`. Both lanes share apply, durable save, journal retirement and proof-bound receipt removal. Application/save errors retain their original types; cleanup errors are wrapped after the save. `ReplyCompletion` loads through `ReplyReceipts`; restart and emergency paths retain their distinct persistence, accounting and error policies |
| `mrs_bot_reply_delivery.py` | Conversational reply transport and durable receipt lifecycle | `ReplyCycleDelivery.deliver` calls cycle-bound `ReplyReceipts`, `ReplyCompletion`, `ReplyReceiptValues`, `TweetLookupCache` and `ApiCooldowns` owners directly. Its post callback retains fresh send-time composition. Operation-scoped `ReplyReceipts` owns loading, exclusive publication and current/legacy promotion, including exact source binding, replacement checks and later-operation refresh. The global write barrier receives the applicable receipt owner directly. Proof-bound exact removal and transport/journal authority are unchanged; lanes own terminal bookkeeping |
| `mrs_bot_normal_reply_cycle.py` | Normal mention and hot-post reply orchestration | Uses fresh context, tweet lookup, generation, history, author-quarantine, evaluation, clarification, accounting, cooldown and runtime-control owners alongside existing typed settings, persistence and delivery boundaries. One cooldown owner is shared with generation, delivery and hot-post discovery; one control owner is shared with discovery. The root composes real discovery operations without their public adapters; X transport/pagination and fresh root backlog continuation remain callbacks. Actual-model-call budgets and one-success-per-cycle policy stay explicit. Owner clock calls, quarantine batching, draft persistence and durable retirement retain their ordering |
| `mrs_bot_quote_reply_cycle.py` | Quote-tweet reply orchestration, eligibility and lane markers | Uses fresh context, tweet lookup, generation, history, evaluation, accounting, cooldown, runtime-control and `QuoteWatchPosts` owners alongside the shared settings, persistence and delivery contracts. Generation and delivery share the cycle's cooldown owner; the watched-post owner shares its tweet-cache owner. Private helpers call owner operations directly; compatibility root functions are outside this path. Watch-list and numeric candidate ordering, reply delay, candidate/author limits, one-success-per-cycle policy and durable bookkeeping retain their existing order |
| `mrs_bot_quote_discovery.py` | Watched/recent own-post selection and durable recent-search quote discovery | `QuoteWatchPosts` owns fresh watch-file reads, recent-original selection and priority merging and calls `TweetLookupCache.seed_recent_own_posts` directly. Discovery saves enriched `quote_pending_candidates` before query-specific cursor advancement and returns unfinished work before further search, independent of watch changes. Terminal quote markers and confirmed reconciliation retire queued targets; pagination callbacks and bounded per-original fallbacks remain supplied |
| `mrs_bot_hot_post_discovery.py` | Watched hot-post discovery, durable skip records and mention/hot candidate handoff | Stages watched tracking-map pruning separately from search, retaining original value references and delayed publication. Calls `QuoteWatchPosts`, `TweetLookupCache`, `ReplyEvaluations`, `ApiCooldowns` and `RuntimeControls` directly; current X pagination, eligibility, draft and persistence boundaries remain supplied. Preserves full rescans, invalid-cursor durable cleanup, eligibility before caps, draft retirement, conservative watermarks and bounded skip/merge behavior |
| `mrs_bot_mention_discovery.py` | Mention discovery, durable queue access and watermark retirement | `MentionQueue` calls `MentionAuthority` directly for recovery, ordered access and monotonic retirement through current sorting and save boundaries. Discovery calls `TweetLookupCache` and `ReplyEvaluations` directly. The traversal coordinator retains X fetching, page budgets and cursor recovery; `_persist_mention_page` commits page candidates and cursor/watermark state with explicit incremental progress. Completion follows durable saving and precedes diagnostics; transient lookup failures retain the queue |
| `mrs_bot_mention_authority.py` | `MentionAuthority`: pagination, backlog/reset guards and canonical page ownership | Validation owns normalization and recovery reporting; state loading still prefers usable backups before identity recovery, and persistence remains with callers. Owns fixed mention continuation grammar and calls fixed terminal evaluation directly; normal-cycle receipt preparation no longer forwards provenance validation |
...
| `mrs_bot_author_quarantines.py` | Mention-author quarantine strikes, expiry, lookup and policy migration | `AuthorQuarantines` owns retention and transitions using current thresholds, policy, clock and logging. Normalization preserves historical migrations without clock access; pure clearing retains surviving record references. Construction is inert and durable saves remain external. Frozen historical evidence-policy identifiers live beside their migrations; the active policy and all runtime expiry/retention inputs remain current. |
| `mrs_bot_tweet_lookup_cache.py` | Verified tweet lookup, normalization, pruning, cached text/media refresh and recent own-post index | `TweetLookupCache` calls its owned normalization, verification, pruning, storage and fetch operations directly, and uses its composed `StateValues` owner for cache epochs. Current request, clock, policy and persistence boundaries bind inertly. Preserves cache-hit references, legacy refresh saves, transient media copies and fresh pre-send lookup |
| `mrs_bot_reply_context.py` | Verified parent paths and canonical normal/quote contexts with separate media | `ReplyContext` separates parent-path admission from usable-turn projection and context assembly. Root-gap diagnostics, contiguity and chronology checks precede projection and media work. It receives `TweetLookupCache` and `ReplyMedia` directly for pruning, bounded parent lookup, direct-quote refresh and prepared native media; those operations bypass root relays. Current limits, lookup budgets, references, copy boundaries and date/log order are preserved. Construction is inert; only canonical context is persisted. |
| `mrs_bot_reply_native_media.py` | Native attachments, photo selection, trusted image retrieval and byte validation | `ReplyMedia` owns selection through collection with distinct context/supplied caps and current MIME policy. Fixed loopback recognition is direct; response metadata is validated in order before streaming. Target priority, source references, redirect/stream bounds, transient errors, closing and validator-result identity are preserved. Discovery and lookup import pure attachment expansion; remote calls retain supplied safeguards |
| `mrs_bot_reply_lane_policy.py` | Deterministic target and spam gates | Owns fixed regex operations while current identity, mutable patterns and logging remain supplied. Preserves eligibility and spam decisions; re-exports clarification constants, thread IDs and daily author-count normalization for compatibility |
| `mrs_bot_daily_reply_accounting.py` | Daily reply buckets, author counts and confirmed-reply increments | `DailyReplyAccounting` owns resets, canonical dates and confirmation advancement and calls its shared `ReceiptDates` directly for current London cap dates. Recovery never rolls newer buckets backward; increments retain the caller's prior idempotency decision and receipt-date snapshots. Mutates explicit state without saves; construction is inert |
| `mrs_bot_reply_clarifications.py` | Clarification eligibility, author windows and completed repair records | Fixed lexical rules and token extraction are local; `ClarificationReplies` owns detection, terminal-thread lookup, confirmation conflicts and recording. It receives `ReplyContext` directly for parent identity, own-reply proof and nested cached refresh, bypassing root relays. Requires cached question and confirmed-reply proof; transient refresh errors defer evaluation. Conflict checking and completion recording retain their separate reconciliation positions, references and event order. Construction is inert; saves and delivery remain external |
| `mrs_bot_runtime_control.py` | `RuntimeControls`: stable snapshots, fail-closed cache results and pause policy | Loading and lane/global decisions call owned snapshot, recovery and boolean operations; reply lanes, hot-post discovery and runtime maintenance call the owner directly while public root adapters remain compatible. The current cache, shared schema validation and runtime clock retain their boundaries. Control parsing directly uses the inert strict runtime JSON owner at the original parse point. Snapshot fingerprint hashing is fixed locally and still occurs only after the descriptor is closed and its stable identity is verified. |
| `runtime_control_contract.py` | Shared bot/digest control keys, timestamp bound and pure validation | Metadata validation rejects unsupported keys and non-integer generation; value validation preserves exact Decimal checks and the bot's existing timestamp rules. Separate phases let the digest retain its clock boundary; callers retain file I/O, parsing, caching and failure reporting. Fixed Decimal/finite-number semantics are local; parser callbacks, supported key sets, epoch limits and date authorities retain their original call boundaries. |
| `mrs_bot_api_cooldowns.py` | API cooldown expiry, error-window pruning and rate-limit/repeated-error policy | `ApiCooldowns` owns internal pruning and rate-limit calculations. Reply cycles share it with nested generation/delivery/discovery, while send-time and runtime-maintenance roots bind fresh owners. Preserves separate clock samples, scope routing and ordinary-save ordering; current dependencies are bound per root call without retaining caller state |
...
| `mrs_bot_state_candidate_validation.py` | State candidate normalization, reader compatibility and meme schedule validation | Candidate normalization receives `StateValues`, `TweetLookupCache`, `MentionAuthority`, `ReplyEvaluations` and `AuthorQuarantines` directly. Legacy/current history collections share ordered validation and shallow retention; recovery callbacks, overflow resets, legacy evaluation retirement and quarantine pruning retain their original order. Fixed receipt-commit grammar comes from the state-generation owner. |
| `mrs_bot_state_loading.py` | Durable state loading and ordered recovery selection | Receives one root-composed candidate normalizer directly. Separates replica selection, rollback-fence validation and selected-candidate recovery reporting. Primary ambiguity stops loading; unusable unneeded backups retain their warning path. Sealed-generation collision checks, newest-replica choice and reporting-before-durable-repair order are preserved. |
| `mrs_bot_main_post_receipts.py` | Main-post attempt/pending/completed receipt grammar and bound schedule materialization | `MainPostReceiptValues` owns attempt validation, payload binding, attempt/confirmation matching, pending construction, completed receipt validation and materialization. Each operation keeps its policy, calendar, epoch and exception references; nested operations bind a fresh owner at the original call boundary. Fixed payload/hash/copy helpers are direct. Preserves legacy formats, exact lineage, bound calendar replay, native errors, copy boundaries and the nonrecursive materialization bypass; durable storage and mutation authority remain external |
| `mrs_bot_main_post_receipt_storage.py` | Main-post receipt paths, storage and durable lifecycle transitions | `MainPostReceipts` owns paths, reads, publication, attempt promotion, schedule finalization and exact removal. Nested storage operations refresh current bindings directly within the owner. A lazy `MainPostReceiptValues` provider supplies receipt grammar and materialization at each original call boundary. Regular/meme writers share ordered publication gates while retaining lane-specific readers and errors. Root removal adapters validate commit proofs and supply a narrowly bound retirement capability; the owner factory grants none |
| `mrs_bot_main_post_attempt_values.py` | Main-post payload identity, meme-plan snapshots, attempts and pending confirmations | Owns fixed bound-plan fields, delay limits, payload reconstruction and canonical JSON hashing, used directly by receipt validation and transport preparation; fixed copying, hashing and post-ID shape checks are local; runtime validators, entropy and clock remain supplied per call |
| `mrs_bot_main_post_confirmation_persistence.py` | Confirmed main-post promotion and protected/emergency persistence | Calls `MainPostReceipts` and `MainPostReceiptValues` directly before binding remote confirmation durably; exact-byte/source checks, incident latching and fsync failures maintain barriers |
| `mrs_bot_main_post_publication.py` | Shared quote/meme publication transaction | `MainPostPublication` calls cycle-bound `MainPostReceipts` and `MainPostReceiptValues` directly for attempt publication, pending construction and finalization, while transport/media handoff, guarded create, non-success proof retirement and confirmation promotion remain real boundaries. Retains replacement attempt and partial pending-receipt results without copying; guard release stays explicit. No import-time work |
| `mrs_bot_x_request.py` | Authenticated X requests and response handling | Enforces route/body/pause/authority checks before transport, handles diagnostics and provider health, and closes proof scopes. Receives invocation-scoped `XRequestRoutes` and `XCreateDiagnostics` owners directly; its entry point has 39 total parameters (32 injected), down from 41 (34). Authentication, request execution, response validation, transport authority and pre-send controls remain supplied boundaries. Route/authority and exact multipart validation keep their original call points; bearer transport is read-only |
| `mrs_bot_post_creation.py` | Receipt-bound media upload, public-post creation and confirmed-media handoff | Owns fixed media metadata, paths and post-ID validation. Main-post validation, payload binding, attempt promotion and path lookup call supplied receipt/value owners directly; `create_post`, journal orchestration and remote-write safeguards remain explicit boundaries. Current exception, pause, rejection, ambiguity, mutation and SIGINT behavior is preserved. |
| `mrs_bot_main_post_reconciliation.py` | Main-post receipt application, emergency representation and local recovery | Receipt application calls invocation-scoped `MemeSchedule` and `TweetLookupCache` owners directly; recovery loads and finalizes through `MainPostReceipts` while proof-authorized removal remains at the root. Authoritative versus stale/legacy replay, future schedule repair, image-history identity and schedule idempotence remain explicit. Regular replay shares the live protected-persistence/outbox/retirement sequence; regular precedes meme after the simultaneous-receipt gate. |
| `mrs_bot_historical_context_delivery.py` | Historical-context delivery, store factories and interrupted-attempt recovery | Uses worker locks, receipt/source proofs and supplied posting callbacks; recovery updates outbox, history and receipt in that order; fixed outbox failure recording and proof verification stay local and are used directly by queue processing |
| `mrs_bot_historical_context_queue.py` | Historical-context enqueueing and queue processing | The coordinator keeps runtime gates, selection, interrupted recovery, claiming and loop decisions. Private helpers bind per-claim delivery callbacks, classify exceptions against current durable transport state and persist returned outcomes. Cooldown saving precedes outcome persistence; unavailable latches and exception boundaries stay explicit |
...
| `mrs_bot_receipt_retirement.py` | Receipt retirement and exact transport source verification | Owns fixed source hashing, digest grammar, path construction and the confirmed-outbox lineage matcher. Checks exact source bytes, attempt ordinals and durable proofs before guarded mutations; current stores, clocks, locks, filesystem and exception authority remain supplied. |
| `mrs_bot_remote_write_barriers.py` | Global remote-write barriers and eligibility for local recovery | Owns fixed paths, hashes, receipt encoding and file-mode interpretation. Inspects current receipts, journals, markers, media and outbox state through supplied runtime boundaries; unsafe or unresolved evidence blocks unrelated writes, with bounded exact-source local recovery. |
| `mrs_bot_local_config.py` | Strict runtime JSON and local-configuration loading/coercion | `LocalConfiguration` owns bounded stable reads, strict decoding, migration/coercion and complete override validation. Runtime application receives the owner directly and calls `load_overrides`; public read/load adapters remain compatible. Runtime validators remain supplied per call; source defaults, applying overrides and bootstrap remain in the root. Fixed parsing and snapshot copying are local; current defaults, validation, filesystem authority and errors remain injected |
| `mrs_bot_runtime_configuration.py` | Runtime configuration paths, validation, application and credential checks | Owns path containment, historical-context schema checks, numeric bounds and cross-field validation. Applies values loaded through its invocation-scoped `LocalConfiguration` while retaining namespace mutation and logging order; the application entry point remains at 5 injected parameters with one typed owner replacing the loader callback. Root path-guard aliases preserve startup order; configuration storage and source-default snapshots remain in the root. Import is inert. |
| `mrs_bot_self_test.py` | Local self-test orchestration and result logging | Owns local JSON decoding; current paths, configuration, loaders and logging remain supplied. Its public local-config loader callback is retained as a non-production diagnostic boundary. Reports local validation failures without entering the production loop or calling providers. |
| `mrs_bot_observability.py` | Logging setup, bounded diagnostics and publication/recovery events | Owns fixed path construction and managed-handler marking/removal, with root compatibility aliases. Configures current handlers and runtime paths, redacts sensitive values and emits descriptive events without authorizing publication or recovery. |
| `mrs_bot_runtime_service_initialisation.py` | Runtime health, lazy reply evidence and historical semantic-gate initialization | Owns fixed evidence-path construction and initializes services only on explicit calls. Coordinator-owned caches, errors and logger stay current; semantic review and live formatting share the same ordered option projection. |
| `mrs_bot_safety_marker_snapshots.py` | Safety-marker snapshots and durability acknowledgement | Owns fixed path and mode rules for bounded no-follow reads, validates marker/successor identity and acknowledges exact bytes under the process lock with strict fsync |
| `mrs_bot_remote_write_incidents.py` | Remote-write incident latching and durable marker acknowledgement | Owns fixed marker encoding and hashing; sets process latches before file work and clears uncertainty only after durable marker acknowledgement, before releasing retained SIGINT |
| `mrs_bot_instance_lock_checks.py` | Instance-lock verification and mutation-authority issuance | Verifies singleton socket, directory/file locks, PID and OFD evidence; issued authority revalidates the live lock before mutations; the fixed inode/device socket-name algorithm is shared directly with the offline reconciler. Owns fixed file-mode/error-number interpretation and the exact OFD record format shared with offline reconciliation; lock syscalls, descriptor checks and mutation authority retain current runtime boundaries |
...
| `mrs_bot_transaction_recovery.py` | Local transaction recovery and startup receipt gates | Owns fixed receipt hashing while coordinating current global gates, receipt selection and journal inspection. Startup reconciliation receives the composed main-post graph; pre-barrier main-post inspection loads through `MainPostReceipts`, while due schedule repair calls `QuoteSchedule` directly. Historical-lane recovery preserves dispatch, receipt identity, attempt ordinals, lock rechecks and native failures. Recovery performs no new remote work. |
| `mrs_bot_transport_source_preparation.py` | Transport source preparation and final receipt gates | Main-post preparation calls `MainPostReceipts` and `MainPostReceiptValues` directly for validation, attempting promotion and source path selection. Current and frozen reply validators still share exact payload matching after distinct receipt checks; transport mutation, retirement, journal, receipt and media barriers remain explicit |
...
| `mrs_bot_used_history.py` | `UsedHistory`: durable quote and image history | Owns stable loading, source-verified migration and conditional rewrites; calls the shared `QuoteCandidates` for current quote hashes and `AssetMetadata` for quote/image analysis. The image normalization algorithm also supports the historical simulator's separate corpus-proof policy. Fixed JSON, regex, hash and basename operations remain local; paths and durable writes retain their existing boundaries. |
| `mrs_bot_receipt_primitives.py` | Receipt scalar values, calendars and confirmation times | Fixed identity/basename/scalar checks use owned operations directly; epoch bounds remain current inputs. One invocation-scoped `ReceiptDates` is shared by conversational receipt projection, reconciliation and daily accounting while public adapters remain compatible. Ambient dates, London cap dates and their safe conversions remain distinct from receipt-bound timezone validation/conversion; conservative confirmation fallback remains explicit |
| `mrs_bot_runtime_state_helpers.py` | Runtime state defaults, load maintenance and scheduling values | Creates fresh defaults and mutates supplied schedule fields; load maintenance calls `ApiCooldowns.clear_expired` directly before priority sanitisation. Epoch validation remains current; `QuoteSchedule` owns current delay selection and optional-save scheduling and is called directly by due ticks and receipt repair. One-shot main-post preparation calls `MemeSchedule` directly; fixed field application remains local |
...
`ReplyCycleDelivery.deliver` in `mrs_bot_reply_delivery` owns pre-send availability,
posting, confirmed/ambiguous error propagation and retryable API accounting.
The root binds its error types and routing callbacks once per cycle; the captured
post callback still composes current runtime dependencies when sending.
