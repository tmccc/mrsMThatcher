...
| `mrs_bot_quote_posting.py` | Ordinary quotation posting and local recovery | Receives one invocation-scoped `ImageSelection`, using its `UsedHistory` gate and `choose_pair` method directly while retaining history rollback, context obligations and recovery projections. A cycle-bound `MainPostPublication` owns shared publication steps; confirmed persistence and receipt retirement use regular-post completion |
...
| `mrs_bot_daily_meme.py` | Daily meme assets, calendar scheduling and transactional posting | `MemeCatalog` owns discovery, summaries and history-reset selection; `MemeSchedule` owns calendar, fallback and quote-anchor operations. Transactional posting receives both owners and calls them directly for selection, summaries, same-day guards and fallback scheduling. `MainPostPublication` shares durable publication; named diagnostics, random-draw order, preparation exception boundaries, schedule projections and emergency state recovery remain unchanged |
...
| `mrs_bot_main_post_receipts.py` | Main-post attempt/pending/completed receipt grammar and bound schedule materialization | `MainPostReceiptValues` owns the six operations and their sibling calls. Each operation keeps its policy, calendar, epoch and exception references; nested operations bind a fresh owner at the original call boundary. Fixed payload/hash/copy helpers are direct. Preserves legacy formats, exact lineage, bound calendar replay, native errors, copy boundaries and the nonrecursive materialization bypass; durable storage and mutation authority remain external |
| `mrs_bot_main_post_receipt_storage.py` | Main-post receipt paths, storage and durable lifecycle transitions | `MainPostReceipts` owns paths, reads, publication, attempt promotion, schedule finalization and exact removal. Nested storage operations refresh current bindings directly within the owner. A lazy `MainPostReceiptValues` provider supplies receipt grammar and materialization at each original call boundary; current-generation authorization and attempt matching remain separate. Regular/meme writers share ordered publication gates while retaining lane-specific readers and errors. Root removal adapters validate commit proofs and supply a narrowly bound retirement capability; the owner factory grants none |
...
| `mrs_bot_main_post_publication.py` | Shared quote/meme publication transaction | `MainPostPublication` binds runtime callbacks once per cycle and owns attempt publication, transport/media handoff, guarded create, non-success retirement and confirmation promotion. Retains replacement attempt and partial pending-receipt results without copying; guard release stays explicit so lane recovery and projection failure boundaries remain unchanged. No import-time work |
...
| `mrs_bot_post_creation.py` | Receipt-bound media upload, public-post creation and confirmed-media handoff | Owns fixed media metadata, paths and post-ID validation. Separates prepared-receipt checks, requested-payload projection and response-identity parsing from journal orchestration. Current exception, pause, rejection, ambiguity, mutation and SIGINT boundaries remain supplied. |
| `mrs_bot_main_post_reconciliation.py` | Main-post receipt application, emergency representation and local recovery | Receipt application calls an invocation-scoped `MemeSchedule` directly for legacy bound-zone dates and quote-anchored fallback. Authoritative versus stale/legacy replay, future schedule repair, image-history identity and schedule idempotence remain explicit; retired spacing fields are left untouched. Regular replay shares the live protected-persistence/outbox/retirement sequence while retaining recovered event fields and native exceptions. Regular precedes meme after the simultaneous-receipt gate; no retries or import-time runtime work |
...
| `mrs_bot_transaction_recovery.py` | Local transaction recovery and startup receipt gates | Owns fixed receipt hashing while coordinating current global gates, receipt selection and journal inspection. Due regular-receipt schedule repair calls `QuoteSchedule` directly without changing save order. Historical-lane recovery shares bound-transport promotion with preserved dispatch, receipt identity, attempt ordinals, lock rechecks and native failures. Recovery performs no new remote work; import retains no authority. |
| `mrs_bot_transport_source_preparation.py` | Transport source preparation and final receipt gates | Current and frozen reply validators share exact payload matching after distinct receipt checks; binds source identities before transport mutation; checks retirement, journal, receipt and media barriers and generation-bound handoffs |
