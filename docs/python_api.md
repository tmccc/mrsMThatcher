...
| `mrs_bot_quote_posting.py` | Ordinary quotation posting and local recovery | `QuotePostRunner` owns selection, preparation, rollback and the live transaction. It calls fresh operation-bound selection, publication and recovery collaborators; confirmed persistence and proof-gated receipt retirement use regular-post completion. |
...
| `mrs_bot_daily_meme.py` | Daily meme assets, calendar scheduling and transactional posting | `DailyMemeRunner` owns meme preparation, named diagnostics and the live transaction. `MemeCatalog` and `MemeSchedule` own discovery, cycle selection and calendar behaviour. Fresh operation-bound publication and recovery retain proof-gated retirement and emergency state recovery. |
...
| `mrs_bot_main_post_assembly.py` | Main-post dependency assembly | Constructs fresh quote and meme runners, receipt values/storage, publication and recovery at each operation boundary. The root supplies settings and genuine shared application authorities; nested value/storage refreshes bind current settings without rebuilding transaction authorities. |
...
| `mrs_bot_main_post_reconciliation.py` | Main-post receipt application, emergency representation and local recovery | `MainPostRecovery` owns receipt application and replay for both lanes with operation-bound storage, values, schedule and cache. Proof-authorised removal remains an external application boundary. Authoritative versus stale/legacy replay, future schedule repair, image-history identity and schedule idempotence remain explicit. Live regular completion and regular replay keep their protected-persistence/outbox/retirement sequences; regular precedes meme after the simultaneous-receipt gate. |
