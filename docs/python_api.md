...
| `mrs_bot_runtime_control.py` | `RuntimeControls`: stable snapshots, fail-closed cache results and pause policy | Loading and lane/global decisions call owned snapshot, recovery and boolean operations; the current cache, shared schema validation and runtime clock retain their boundaries. Control parsing directly uses the inert strict runtime JSON owner at the original parse point |
