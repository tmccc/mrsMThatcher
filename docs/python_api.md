...
| `mrs_bot_daily_meme.py` | Daily meme selection, calendar scheduling and transactional posting | Selects and publishes through current callbacks; lane-specific confirmed schedules support normal and emergency persistence  `MemeSchedule` owns current calendar, date guards, fallback and quote-anchor operations; root adapters bind current policy and preserve optional saves and clock order. Durable receipt replay keeps its bound schedule plan. |
