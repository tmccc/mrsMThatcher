...
| `mrs_bot_reply_drafts.py` | Pending reply draft validation, storage, recovery and retirement | `ReplyDrafts` owns the full draft lifecycle, using current evidence, validators, result vocabulary and telemetry. Local-validation failure retires the draft before a dedicated zero-call diagnostic result is built and recorded. Preserves draft copies, retained references, recovery/error distinctions and target-wide clearing; durable saves and receipt authority remain external |
...
| `mrs_bot_quote_discovery.py` | Watched/recent own-post selection and batched recent-search quote discovery | `QuoteWatchPosts` owns fresh watch-file reads, recent-original selection and priority merging. Discovery owns fixed cursor hashing and validation operations, retaining supplied pagination callbacks, query-specific continuations and bounded per-original fallbacks |
