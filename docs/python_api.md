...
| `mrs_bot_reply_context.py` | Verified parent paths and canonical normal/quote contexts with separate media | `ReplyContext` separates parent-path admission from usable-turn projection and context assembly. Root-gap diagnostics, contiguity and chronology checks precede projection and media work. Fixed completeness checks use the lookup owner; current limits, lookup budgets, references, copy boundaries and date/log order are preserved. Construction is inert; only canonical context is persisted. Receives `TweetLookupCache` directly for pruning, bounded parent lookup and direct-quote media refresh; those operations bypass root relays. |
...
| `mrs_bot_reply_clarifications.py` | Clarification eligibility, author windows and completed repair records | Fixed lexical rules and token extraction are local; `ClarificationReplies` owns detection, terminal-thread lookup, confirmation conflicts and recording. Requires cached question and confirmed-reply proof; transient refresh errors defer evaluation. Conflict checking and completion recording retain their separate reconciliation positions, references and event order. Construction is inert; saves and delivery remain external. Fixed tweet-completeness checks use the lookup owner directly |
