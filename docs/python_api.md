...
| `mrs_bot_reply_drafts.py` | Pending reply draft validation, storage, recovery and retirement | `ReplyDrafts` owns the full draft lifecycle, using current evidence, validators, result vocabulary and telemetry. Storage checks current confirmed replies before retaining a fresh draft. Recovery's local-validation failure retires the draft before a dedicated zero-call diagnostic result is built and recorded. Preserves draft copies, retained references, recovery/error distinctions and target-wide clearing; durable saves and receipt authority remain external |
...
| `mrs_bot_reply_receipt_values.py` | Conversational receipt validation, send-template preparation, source reconstruction and attempt/confirmation times | `ReplyReceiptValues` owns current/legacy lifecycle dispatch and calls a shared invocation-scoped `ReceiptDates` directly for safe attempt and London confirmation dates. Preserves shallow source references and hashes; conservative confirmation uses the supplied clock/logger. Construction is inert; durable I/O and retirement authority remain external. Fixed scalar validation, mention continuation grammar and frozen legacy strategy identifiers come directly from their inert owners; clocks, epoch policy, draft validation and errors remain current inputs |
| `mrs_bot_reply_reconciliation.py` | Confirmed reply state application, shared completion, restart reconciliation and emergency completeness | Confirmed application shares one invocation-scoped `ReceiptDates` with `ReplyReceiptValues` and `DailyReplyAccounting`, calling it directly only for legacy missing-date fallback. Both reply lanes share apply, durable save, journal retirement and receipt removal. Application/save errors retain their original types; cleanup errors are wrapped after the save. Restart reconciliation keeps its separate persistence policy and exactly-once accounting; `ReplyCompletion` owns shared durable commit and ordered retirement while fresh, restart and emergency paths keep distinct error policies |
| `mrs_bot_reply_delivery.py` | Conversational reply transport and durable receipt lifecycle | `ReplyCycleDelivery.deliver` owns pre-send checks and error routing with cycle-bound authorities and calls `ApiCooldowns.record_error` directly; its post callback retains fresh send-time composition and its own cooldown owner. Operation-scoped `ReplyReceipts` owns loading, exclusive publication and current/legacy promotion, including exact source binding and replacement checks. Delivery binds it at each operation and nested load. Proof-bound exact removal retains its root authority callback; `ReplyCompletion` owns commit and retirement, and lanes own terminal bookkeeping |
