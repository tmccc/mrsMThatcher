...
| `mrs_bot_quote_posting.py` | Ordinary quotation posting and local recovery | Selects and publishes through current callbacks; fixed post-ID validation and state-field application use their owners directly. Confirmed persistence and receipt retirement use regular-post completion; separate recovery projections track partial progress |
