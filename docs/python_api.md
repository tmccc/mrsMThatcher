...
| `mrs_bot_reply_state.py` | Confirmed conversational history queries and ineligible-target draft retirement | Shared retirement logs pending-draft metadata, clears the draft and records a terminal evaluation through current callbacks; callers retain eligibility, lane bookkeeping and persistence. History queries preserve row references and ordering |
