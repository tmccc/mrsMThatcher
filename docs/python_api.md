...
| `mrs_bot_reply_context.py` | Verified parent paths and canonical normal/quote contexts with separate media | `ReplyContext` owns lookup policy, path verification, usable parent-suffix projection, participant roles and both context builders. Fixed completeness checks use the lookup owner; current visible/incoming limits remain separate from the fixed post default. Post row references, independent copies, media references and date/log order are preserved. Construction is inert; returns PreparedReplyContext while only canonical context is persisted |
