...
| `mrs_bot_state_candidate_validation.py` | State candidate normalization, reader compatibility and meme schedule validation | Scalar normalization resolves the existing StateValues owner per operation; recovery callbacks, overflow resets, legacy evaluation retirement and quarantine pruning retain their original order; fixed receipt-commit record grammar comes from the state-generation owner beside its writer and commit proofs |
