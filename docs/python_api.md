...
| `mrs_bot_reply_generation.py` | Responses transport, typed reply evaluation and outcome accounting | Returns the full PipelineResult while preserving retry policy, prior-429 metadata and separate provider-health/candidate dispositions. Image collection uses the external media owner; explicit calls may call the provider and account for failures through current root callbacks. No import-time runtime work |
