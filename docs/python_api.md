...
| `mrs_bot_reply_generation.py` | Reply pipeline evaluation, failure dispositions and decision telemetry | `ReplyGeneration` receives `ReplyMedia`, `ReplyHistory` and `ReplyModelTransport` directly and calls their collection, selection, transport and provider-error operations without root relays. Failed media uses the original target/visible-turn snapshot and records the zero-call result before warning, without history/provider access. Current pipeline, evidence, pause, error vocabulary and telemetry boundaries remain supplied; result references, actual-call accounting and cooldown separation are preserved |
...
| `mrs_bot_reply_delivery.py` | Conversational reply transport and durable receipt lifecycle | `ReplyCycleDelivery.deliver` owns pre-send checks and error routing with cycle-bound authorities; its post callback retains send-time composition. Operation-scoped `ReplyReceipts` owns loading, exclusive publication and current/legacy promotion, including exact source binding and replacement checks. Delivery binds it at each operation and nested load. Proof-bound exact removal retains its root authority callback; `ReplyCompletion` owns commit and retirement, and lanes own terminal bookkeeping |
| `mrs_bot_normal_reply_cycle.py` | Normal mention and hot-post reply orchestration | Uses fresh context, tweet lookup, generation, history, author-quarantine, evaluation, clarification and accounting owners alongside existing typed settings, persistence and delivery boundaries. The root composes real mention and hot-post discovery operations without their public adapters; X transport/pagination and fresh root backlog continuation remain callbacks. Actual-model-call budgets and one-success-per-cycle policy stay explicit. Owner clock calls, quarantine batching, draft persistence and durable retirement retain their ordering |
| `mrs_bot_quote_reply_cycle.py` | Quote-tweet reply orchestration, eligibility and lane markers | Uses fresh context, tweet lookup, generation, history, evaluation, accounting and `QuoteWatchPosts` owners alongside the shared settings, persistence and delivery contracts. The watched-post owner shares the cycle's tweet-cache owner. Private helpers call owner operations directly; compatibility root functions are outside this path. Watch-list and numeric candidate ordering, reply delay, candidate/author limits, one-success-per-cycle policy and durable bookkeeping retain their existing order |
...
| `mrs_bot_runtime_control.py` | `RuntimeControls`: stable snapshots, fail-closed cache results and pause policy | Loading and lane/global decisions call owned snapshot, recovery and boolean operations; the current cache, shared schema validation and runtime clock retain their boundaries. Control parsing directly uses the inert strict runtime JSON owner at the original parse point. Snapshot fingerprint hashing is fixed locally and still occurs only after the descriptor is closed and its stable identity is verified. |
...
| `mrs_bot_runtime_state_helpers.py` | Runtime state defaults, load maintenance and scheduling values | Creates fresh defaults and mutates supplied schedule fields; epoch validation and lazy scheduling use current callbacks; `QuoteSchedule` owns current delay selection and optional-save scheduling, with fixed field application local |
