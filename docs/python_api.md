...
| `mrs_bot_reply_state.py` | Ineligible-target draft retirement and compatibility aliases | Shared retirement logs pending-draft metadata, clears the draft and records a terminal evaluation through current callbacks; callers retain eligibility, lane bookkeeping and persistence |
| `mrs_bot_reply_generation.py` | Typed reply evaluation and outcome accounting | Coordinates media, history, evidence/pipeline and model transport through current callbacks. Returns the full PipelineResult and preserves cooldown ordering, prior-429 accounting, telemetry and distinct provider-health/candidate dispositions. No import-time runtime work |
...
| `mrs_bot_quote_reply_cycle.py` | Quote-tweet reply orchestration, eligibility, context and lane markers | Uses fresh evaluation and accounting owners alongside the shared settings, persistence and delivery contracts. Watch-list and numeric candidate ordering, reply delay, candidate/author limits and one-success-per-cycle policy remain explicit. Preparation, provenance, attempt binding and durable terminal bookkeeping retain their existing order |
...
| `mrs_bot_reply_context.py` | Verified parent paths and canonical single-call context with separately prepared media | `ReplyContext` owns parent/quote lookup policy, path verification, participant roles and context assembly. Its operations call each other directly with bounded cache lookups and separate media preparation; returns PreparedReplyContext while canonical context alone is persisted. Construction binds current external capabilities without runtime access |
