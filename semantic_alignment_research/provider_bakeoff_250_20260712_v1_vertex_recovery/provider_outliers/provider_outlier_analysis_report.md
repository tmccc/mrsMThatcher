...
- 250 intended frozen cases; 250 have all four results.
- Gemini completed 250 and exhausted 0; missing cases remain explicit.
...
| gemini | 15 | 0 | 20.8% | 76.8% |

## Gemini-only keeps

There are **15** Gemini-only keeps. Gemini's primary relationships in these cases are `{'illustrates_broader_principle': 12, 'related_ideological_substitution': 1, 'direct_illustration': 1, 'illustrates_mechanism': 1}`.

Median Gemini-minus-other-provider score differences:

- `relevance_score`: +23.0 (mean absolute gap 24.1).
- `directness_score`: +35.0 (mean absolute gap 35.3).
- `mechanism_alignment_score`: +25.0 (mean absolute gap 27.4).
- `consequence_alignment_score`: +30.0 (mean absolute gap 26.5).
- `principle_alignment_score`: +25.0 (mean absolute gap 26.6).
- `specificity_score`: +35.0 (mean absolute gap 30.7).
- `editorial_power_score`: +25.0 (mean absolute gap 29.4).
- `overall_suitability_score`: +33.0 (mean absolute gap 35.7).
...
| gemini | 250 | 92.8% | 80.8%/95.5% | 99.0%/95.0% | 10 | 2 |

Outlier-subset evidence:

- grok only replace: 0/1 agreed with Tony; 95% Wilson interval 0.0%–79.3%.
- gemini only keep: 8/15 agreed with Tony; 95% Wilson interval 30.1%–75.2%.

## Meta-critic interaction

Only 16 operational-outlier cases currently have human decisions. The review queue preserves meta decisions, deferrals and disagreement bands for every outlier; no meta rule was changed.
For Gemini-only keeps, meta decisions were `{'unsure': 2, 'replace': 13}`; 2 were deferred. Disagreement bands were `{'moderate': 15}`. Of 15 human-labelled examples, Tony agreed with Gemini 8 time(s) and with the replace majority 7 time(s).
...
| 378fbf96d18c2e10f5a3 | gemini=keep | 3_replace_1_keep | replace | replace | majority (replace) |
...
| gemini | 20.8% | 76.8% | 2.4% | 18.0% | 0.8% | 26.0% | 6.0% | 36.4 | 7.02s | $0.0099 |
...
The generated queue contains 21 informative cases and now carries all available human labels.
