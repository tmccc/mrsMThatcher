...
readonly PROSPECTIVE_CONVERSATION_DIR="${MRS_PROSPECTIVE_CONVERSATION_DIR:-/disks/disk1/research/mrsMThatcher-prospective-conversations-v7}"
...
print_enable_commands() {
  printf '%s\n' \
    ...
    'prospective-conversation activation is deliberately omitted; complete the documented v6-to-v7 rebuild, validation, manual oneshot, and corpus inspection first'
}
...
prepare_scheduled_task_state() {
  ...
  if [[ -e "${PROSPECTIVE_CONVERSATION_DIR}" ]]; then
    ...
    printf 'left prospective conversation v7 root absent for registered rebuild: %s\n' \
      "${PROSPECTIVE_CONVERSATION_DIR}"
  fi
}
