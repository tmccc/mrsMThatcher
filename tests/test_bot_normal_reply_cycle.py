...
def guarded_import(name, *args, **kwargs):
    if name in {'mrsMThatcher2', 'requests', 'openai', 'single_call_reply', 'reply_evidence'} or name.startswith('mrs_bot_') and name not in {'mrs_bot_normal_reply_cycle', 'mrs_bot_runtime_state_helpers', 'mrs_bot_reply_context', 'mrs_bot_reply_cycle_interfaces', 'mrs_bot_reply_preparation', 'mrs_bot_reply_generation', 'mrs_bot_reply_native_media', 'mrs_bot_reply_delivery', 'mrs_bot_reply_state', 'mrs_bot_reply_drafts', 'mrs_bot_reply_history', 'mrs_bot_reply_evaluation_state', 'mrs_bot_author_quarantines', 'mrs_bot_daily_reply_accounting', 'mrs_bot_mention_authority', 'mrs_bot_state_value_normalisation', 'mrs_bot_durable_json_io'}:
        forbidden()
...
def test_adapter_forwards_current_dependencies_arguments_results_and_errors(monkeypatch):
    ...
    assert sum(param.kind is inspect.Parameter.KEYWORD_ONLY for param in parameters.values()) == 45
    removed = {
