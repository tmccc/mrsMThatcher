...
def guarded_import(name, *args, **kwargs):
    if name in {'mrsMThatcher2', 'requests', 'openai', 'single_call_reply', 'reply_evidence'} or name.startswith('mrs_bot_') and name not in {'mrs_bot_reply_clarifications', 'mrs_bot_reply_native_media', 'mrs_bot_request_route_values', 'mrs_bot_tweet_lookup_cache'}:
        forbidden()
...
def confirmed_question(monkeypatch):
    ...
    bot._reply_assembly()._confirmed_reply_state_applier()(state, unit_confirmed_reply_receipt(
...
def test_cached_clarification_keeps_original_references_and_current_correction_token_gate(monkeypatch, make_owner, confirmed_question):
    ...
    trace = Mock()
    contexts = bot._reply_assembly()._reply_context_owner()
...
def test_clarification_requires_ledger_and_cache_proof_and_catches_only_current_parent_error(monkeypatch, make_owner, confirmed_question):
    state, candidate = confirmed_question
    parent = Mock(wraps=bot._reply_assembly()._reply_context_owner().parent_id)
